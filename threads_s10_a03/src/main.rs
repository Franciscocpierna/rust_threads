...
Nesta aula:
Como calcular números primos de forma paralela ?
...
fn main() {
	...
	// Expande os primos conhecidos de até 10_000 para até 10_000_000, poderia ser 100_000_000
	// Divide o trabalho em 4 partes, para 4 threads
	let start = Instant::now();

	let mut novos_primos_conhecidos = primos_conhecidos.clone();

	thread::scope( |scope| {
		let mut handles = Vec::new();

		handles.push( scope.spawn( || {gera_primos_v1(&primos_conhecidos,
										10_001, 2_500_000)
									} ) );
		handles.push( scope.spawn( || {gera_primos_v1(&primos_conhecidos, 
										2_500_001, 5_000_000)
		...
		for h in handles.into_iter() {
			let mut novos = h.join().unwrap();	// Espera a thread filha terminar
			novos_primos_conhecidos.junta(&mut novos);				// Junta os novos aos primos conhecidos
		}
	});

	let duration = Instant::now() - start;
	println!("\nNovos primos conhecidos: {} -> {}, exemplo qualquer {}", 
												novos_primos_conhecidos.inicio_intervalo,
												novos_primos_conhecidos.fim_intervalo,
												novos_primos_conhecidos.primos[123456]);
